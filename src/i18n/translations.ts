      ...
      jobs: [
        {
          ...
          achievements: [
            ...
            '設計並開發基於 JSON Lines (NDJSON) 的 HTTP 串流 (Streaming) APIs，將大數據計算拆分為增量區塊（Incremental Chunks）並即時 Pipe 至用戶端，優化事件循環（Event Loop）效能與記憶體佔用，並顯著縮短首位組傳輸時間 (TTFB)。',
      ...
      jobs: [
        {
          ...
          achievements: [
            ...
            'Performance Optimization: Designed and implemented HTTP streaming APIs using JSON Lines (NDJSON), partitioning monolithic calculations into incremental chunks to stream results instantly, optimizing memory usage and event-loop efficiency while lowering TTFB.',
